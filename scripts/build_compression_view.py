"""Run an interactive Dash app for projected embedding radar charts under compression.

Usage:
    python scripts/build_compression_view.py [--run-dir <path>] [--initial-dimensions <int>]

The app reads embeddings.parquet from the cross-lingual compression run and renders
one interactive Plotly radar chart per canonical text. Cards are ordered by text so
that the same passage can be watched across the five languages as it is progressively
compressed (1.00 -> 0.125). A compression-level control lets you view a single level
or overlay several levels of the per-language trajectory.
"""
...
COMPRESSION_LEVELS = (1.00, 0.50, 0.25, 0.125)
THEME_KEYS = (
    "paper",
    "surface",
    "ink",
...
# Line styling per compression level to disambiguate overlaid trajectories.
LEVEL_STYLE = {
    1.00: {"dash": "solid", "alpha": 0.95},
    0.50: {"dash": "dash", "alpha": 0.75},
    0.25: {"dash": "dot", "alpha": 0.65},
    0.125: {"dash": "longdash", "alpha": 0.6},
}
...
def load_reference_embeddings(reference_run_dir: Path) -> pd.DataFrame:
    """Load the baseline run used to anchor the compression projection."""
    if not reference_run_dir.is_dir():
        raise FileNotFoundError(f"reference run directory not found: {reference_run_dir}")
    parquet_path = reference_run_dir / "embeddings.parquet"
    if not parquet_path.is_file():
        raise FileNotFoundError(f"embeddings.parquet not found at {parquet_path}")
    ...
    if missing_columns:
        missing = ", ".join(sorted(missing_columns))
        raise ValueError(f"reference embeddings.parquet is missing required columns: {missing}")
    if frame.empty:
        raise ValueError(f"reference embeddings.parquet contains no rows: {parquet_path}")
    if "compression_level" not in frame.columns:
        frame = frame.copy()
        frame["compression_level"] = 1.0
    return frame


def build_compression_data(
    frame: pd.DataFrame,
    reference_frame: pd.DataFrame | None = None,
) -> CompressionData:
    """Validate embeddings and precompute the shared PCA coordinate system.

    If a reference frame is provided, the PCA basis is fit on that reference so
    the compression run can be compared directly to the translation baseline.
    """
    ...
    basis_frame = reference_frame if reference_frame is not None else frame
    basis_vectors = [np.asarray(vector, dtype=float) for vector in basis_frame["embedding"]]
    basis_matrix = np.stack(basis_vectors)
    scaled_basis_matrix = StandardScaler().fit_transform(basis_matrix)

    pca_dimension = min(len(basis_frame) - 1, embedding_dimension)
    if pca_dimension < 10:
        raise ValueError("at least 11 embeddings are required for a 10-component PCA view")
    pca = PCA(n_components=pca_dimension)
    pca.fit(scaled_basis_matrix)
    scaled_matrix = StandardScaler().fit(basis_matrix).transform(embedding_matrix)
...
def project_row(
    ...
    if projection_method == "pca":
        if display_dimensions > compression_data.pca_dimension:
            raise ValueError(
                f"PCA supports at most {compression_data.pca_dimension} components, "
                f"got {display_dimensions}"
            )
    ...
    if projection_method == "blocks":
        return project_coordinate_blocks(
            row["embedding"],
            compression_data.center_vector,
            display_dimensions,
        )
...
def build_projection_labels(
    ...
    if projection_method == "pca":
        if not 1 <= display_dimensions <= compression_data.pca_dimension:
            raise ValueError(
                f"display_dimensions must be between 1 and "
                f"{compression_data.pca_dimension}, got {display_dimensions}"
            )
        return tuple(f"PC{component}" for component in range(1, display_dimensions + 1))
    if projection_method == "blocks":
        return build_block_labels(compression_data.embedding_dimension, display_dimensions)
    raise ValueError(f"unknown projection method: {projection_method!r}")
...
def _rgba_with_alpha(rgba: str, alpha: float) -> str:
    """Convert a literal rgba(...) string to one using a new alpha value."""
    if not rgba.startswith("rgba("):
        return rgba
    inner = rgba[5:-1].split(",")
    if len(inner) == 4:
        inner[3] = str(alpha)
    return "rgba(" + ", ".join(inner) + ")"
...
def build_compression_figure(
    ...
    """Build an interactive Plotly radar chart for one canonical text at selected levels."""
    figure = go.Figure()
    for level in sorted(selected_levels, reverse=True):
        level_rows = rows[rows["compression_level"] == level]
        if level_rows.empty:
            continue
        level_style = LEVEL_STYLE.get(level, {"dash": "solid", "alpha": 0.8})
        for language in COMPARISON_LANGUAGES:
            if language not in selected_languages:
                continue
            language_rows = level_rows[level_rows["language"] == language]
            ...
            figure.add_trace(
                go.Scatterpolar(
                    r=vector,
                    theta=closed_labels,
                    name=f"{style['label']} ({language}) {LEVEL_LABELS[level]}",
                    mode="lines+markers" if show_detail else "lines",
                    fill="toself",
                    fillcolor=_rgba_with_alpha(style["background"], level_style["alpha"]),
                    opacity=level_style["alpha"],
...
def build_chart_cards(
    ...
    selected_levels: list[float] | None,
) -> tuple[list[Any], str]:
    ...
    levels = [float(level) for level in (selected_levels or [])]
    filtered = cast(
        pd.DataFrame,
        compression_data.frame.loc[
            compression_data.frame["category"].isin(categories)
            & compression_data.frame["text_id"].isin(text_ids)
        ],
    )
    if not languages:
        return [], "Select at least one language to display charts."
    if not levels:
        return [], "Select at least one compression level to display charts."
    if filtered.empty:
    ...
    plotted_rows = cast(
        pd.DataFrame,
        filtered.loc[
            filtered["language"].isin(languages) & filtered["compression_level"].isin(levels)
        ],
    )
    if plotted_rows.empty:
        return [], "No embeddings match the selected text, language, and level filters."
    ...
    for figure_number, text_id in enumerate(sorted(filtered["text_id"].unique()), start=1):
        ...
        visible_rows = cast(
            pd.DataFrame,
            rows.loc[rows["language"].isin(languages) & rows["compression_level"].isin(levels)],
        )
        if scale_mode == "local":
        ...
        figure = build_compression_figure(
            rows,
            dimension_labels,
            languages,
            levels,
        ...
        if projection_method == "pca":
            explained_ratio = compression_data.pca_explained_variance_ratio[:display_dimensions]
            explained = explained_ratio.sum() * 100
            caption = (
            ...
            caption = (
                ...
                f"R{compression_data.embedding_dimension} -> R{display_dimensions}; "
                f"centered orthogonal blocks - {scale_mode} radial scale."
            )
        level_caption = " + ".join(LEVEL_LABELS[level] for level in sorted(levels, reverse=True))
        cards.append(
            html.Section(
                [
                    html.Header(
                        [
                            ...
                            html.P(f"compression: {level_caption}", className="compression-note"),
                        ],
                    ...
                    dcc.Graph(
                        ...
                        config={
                            "displaylogo": False,
                            "responsive": True,
                            "toImageButtonOptions": {"filename": f"{text_id}_compression_radar"},
                        },
                    ),
    ...
    level_label = " + ".join(LEVEL_LABELS[level] for level in sorted(levels, reverse=True))
    range_note = (
    ...
    summary = (
        f"{len(cards)} text(s) - {projection_label} projection - "
        f"levels: {level_label} - {range_note}."
    )
    return cards, summary


def create_app(compression_data: CompressionData, run_dir: Path, initial_dimensions: int) -> Dash:
    ...
    text_ids = sorted(compression_data.frame["text_id"].unique())
    categories = sorted(compression_data.frame["category"].unique())
    languages = [
        language
        for language in COMPARISON_LANGUAGES
        if language in compression_data.frame["language"].unique()
    ]
    levels = sorted(
        (float(level) for level in compression_data.frame["compression_level"].unique()),
        reverse=True,
    )
    if not 10 <= initial_dimensions <= compression_data.pca_dimension:
        raise ValueError(
            f"initial_dimensions must be between 10 and {compression_data.pca_dimension}, "
    ...
    app = Dash(
        __name__,
        title="Compression Radar Charts",
    ...
    controls = html.Div(
        [
            ...
            html.Div(
                [
                    html.Label("Compression level", htmlFor="compression-filter"),
                    dcc.Dropdown(
                        id="compression-filter",
                        options=[
                            {"label": LEVEL_LABELS[level], "value": level}
                            for level in levels
                        ],
                        value=[1.00] if 1.00 in levels else [levels[0]],
                        multi=True,
                    ),
                ],
                className="filter-field",
            ...
            html.Div(
                [
                    ...
                    dcc.Slider(
                        id="dimension-slider",
                        min=10,
                        max=compression_data.pca_dimension,
                        step=1,
                        value=initial_dimensions,
                        marks={
                            value: str(value)
                            for value in (10, 20, 32, compression_data.pca_dimension)
                            if value <= compression_data.pca_dimension
                        },
                ...
                [
                    ...
                    html.Dl(
                        [
                            ...
                            html.Dt("embeddings:"),
                            html.Dd(str(compression_data.embedding_count)),
                            html.Dt("dimensions:"),
                            html.Dd(str(compression_data.embedding_dimension)),
                            html.Dt("projection:"),
                            html.Dd("global_pca"),
                            html.Dt("compression:"),
                            html.Dd(level_label),
                            html.Dt("centroid_norm:"),
                            html.Dd(f"{compression_data.center_norm:.4f}"),
                        ],
    ...
    def update_dimension_control(
        ...
        maximum = (
            compression_data.pca_dimension
            if projection_method == "pca"
            else compression_data.embedding_dimension
        )
        candidates = (
            (10, 20, 32, compression_data.pca_dimension)
            if projection_method == "pca"
            else (10, 32, 64, 128, 256, 512, compression_data.embedding_dimension)
        )
    ...
    @app.callback(
        ...
        Input("compression-filter", "value"),
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run interactive compression radar charts")
...
def main() -> None:
    ...
    reference_frame = load_reference_embeddings(args.reference_run_dir)
    compression_data = build_compression_data(frame, reference_frame=reference_frame)

    print(f"Loaded {len(frame)} embeddings from {args.run_dir / 'embeddings.parquet'}")
    print(f"Embedding dimensions: {compression_data.embedding_dimension}")
    print(f"Initial display dimensions: {args.initial_dimensions}")
    print(f"Projection reference: {args.reference_run_dir}")

    app = create_app(compression_data, args.run_dir, args.initial_dimensions)
